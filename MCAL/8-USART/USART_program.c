...
u8 USART_u8ReceiveBufferSynch(u8* Copy_u8Buffer, u8 Copy_u8BufferSize) {
    ...
    if (USART_u8State == IDLE) {
        if (Copy_u8Buffer != NULL) {
            for (u8 Local_u8Iterator = 0; Local_u8Iterator < Copy_u8BufferSize; Local_u8Iterator++) {
                u16 Local_u16ReceivedData;
                Local_u8ErrorState = USART_u8ReceiveCharSynch(&Local_u16ReceivedData);

                if (Local_u8ErrorState != OK) {
                    break;  // If there is an error, exit the loop
                }

                // Store only the lower 8 bits of the received 16-bit data
                Copy_u8Buffer[Local_u8Iterator] = (u8)(Local_u16ReceivedData & 0xFF);
            }
        } else {
            Local_u8ErrorState = NULL_PTR;
        }
